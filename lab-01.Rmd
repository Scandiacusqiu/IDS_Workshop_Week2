...
**NA states that the data is not calculable, because there is NA values in the set. If the function returns an warning or an error, then there is at least a character in the set**
...
**na.rm removes ignores the NA while performing the calculation**
...
**The data is very concentrated when is smaller than median, but descrete when the data is larger than the median, meaning the data below median is close to median, but the data greater than median is much greater than median**
...
visitors_with_nations %>% 
  ...
  group_by(nation) %>% 
  count(admission) %>% 
  reframe(admission = admission, n = n, total = sum(n)) %>% 
  filter(admission == "Free") %>% 
  summarise(nation = nation, proportion_of_free = n*100/total)
```
...
visitors %>% 
  ...
  select(region, admission, n_2021, n_2022) %>% 
  mutate(
    percentage_change = (n_2022-n_2021)*100/n_2021
  ) %>% 
  group_by(admission) %>% 
  summarise(mean_percentagechange = mean(percentage_change, na.rm = TRUE)) %>% 
  arrange(mean_percentagechange)
```
